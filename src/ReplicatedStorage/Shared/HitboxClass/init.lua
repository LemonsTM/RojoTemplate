...
local projectileFolder = workspace:WaitForChild('Ignore')
local velocityConstant = 6
...
local function SetupClients()
	local newRemoteEvent: RemoteEvent = Instance.new("RemoteEvent"):: RemoteEvent
	newRemoteEvent.Name = "HitboxClassRemote"
	newRemoteEvent.Parent = ReplicatedStorage
	HitboxRemote = newRemoteEvent

	local newLocalScript: LocalScript = script.HitboxClassLocal:Clone():: LocalScript
	local newSignalModule: ModuleScript = script.Signal:Clone():: ModuleScript
	...
	task.spawn(function()
		for _, Player in Players:GetChildren() do
			...
			local newcsriptClone = newLocalScript:Clone()
			newcsriptClone.Parent = ScreenGUI
			newcsriptClone.Enabled = true
...
if RunService:IsServer() then
	...
	HitboxRemote = ReplicatedStorage:FindFirstChild("HitboxClassRemote"):: RemoteEvent
	if not HitboxRemote then
		...
		HitboxRemote = ReplicatedStorage:WaitForChild("HitboxClassRemote"):: RemoteEvent
	end
end
...
function Hitbox.new(HitboxParams : Types.HitboxParams)
	...
	if HitboxParams.UseClient then
		...
		elseif type(HitboxParams.SizeOrPart) == "number" then
			...
			assert(self.Part and self.Part:IsA("Part"), 'Well shucks thats not good')
...
function Hitbox:Start()
	...
	elseif self.Mode == "Magnitude" then
		...
		self.RunServiceConnection = RunService.Heartbeat:Connect(function()
...
function Hitbox:SetPosition(newPosition : CFrame)
	...
	if velocityConstant then
		constant = velocityConstant or 6
	else
		constant = 6
	end
...
function Hitbox:_GeneratePart()
	...
	elseif type(self.Size) == "number" then
		...
		assert(self.Part and self.Part:IsA("Part"), "Rahhhhhhhhhhh this is comsmos's fault")
